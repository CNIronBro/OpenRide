...
一个完整的网约车后端系统，包含**乘客端、司机端、管理员端**三端 Web 应用。实现了乘客下单 → 智能派单调度 → 司机接单 → 行程跟踪 → 动态计价 → 支付评价的**完整业务闭环**。
...
- 订单不再直接入队派发，而是先写入 **Redis ZSET 等待池**（`order:waiting:pool`），由 `GlobalDispatchScheduler` 每 2 秒统一取出所有待派订单批量处理；
...
### 2. 异步调度引擎：等待池 + Tick 调度 + 延迟轮转

**问题**：下单接口需要快速响应，而司机召回、评分计算、匹配推送是耗时操作，同步处理会阻塞用户请求。

**方案**：将下单与派单流程彻底解耦，构建异步调度引擎：

- **解耦架构**：乘客下单仅写库 + 写入 Redis 等待池即返回，核心派单逻辑异步完成；
- **Tick 调度器**：`GlobalDispatchScheduler` 每 2 秒触发一次，从等待池统一取出所有待派订单，完成 GEO 召回 → 评分 → 匹配 → 推送的全流程；
...
- **漂移检测状态机（V5）**：使用 Haversine 公式计算相邻两次上报的球面距离，5s 内位移超过 200m 判定为漂移点。引入滑动候选坐标（`candidate`）解决早期版本在司机持续移动后陷入"永远无法恢复"的死锁问题——进入 DRIFTING 态后，第一个合理点存入 `candidate`，后续点与 `candidate` 互比；连续两个相邻点距离合理则恢复 NORMAL 态，再次跳变则 `candidate` 滑动到当前点重新积累。整个状态机仅用 3 个 Redis key 驱动，无数据库依赖；
- **假在线检测**：司机每次上报位置时刷新心跳 key（TTL=30s），xxl-job 每 30s 扫描心跳过期的在线司机，从 GEO 集合移除并将数据库状态置为离线。
...
graph TD
    ...
    B --> C[GlobalDispatchScheduler 每 2s tick]
    C --> D[取等待池全部订单]
...
#    管理员端：  http://localhost:8080/admin/index.html  （admin / admin123）
...
├── controller/      # HTTP 入口层（薄层），三端 API 控制器
├── service/         # 业务逻辑层
...
| cleanFakeOnlineDrivers | `*/30 * * * * ?` | 每 30s 清理心跳过期的假在线司机 |
